...
USBHandle::USBHandle(libusb_context* ctx, libusb_device *const dev, const int interface) 
: ctx(ctx), interface(interface) 
{
	...
	// Apparently detaching the USB device's kernel driver is a required step on Linux.
...
void USBHandle::submit_control_transfer(
    ...
    libusb_fill_control_transfer(
        ...
        handle,
        buffer,
        USBHandle::control_transfer_handler,
        this,
        timeout
    );
    std::copy(
...
void USBHandle::control_transfer_handler(libusb_transfer* transfer) {
    if (transfer->status == LIBUSB_TRANSFER_COMPLETED) {
		...
		// TODO it's probably still in the map. How do we access it from here and erase it?
		// Or do you not need to? Why do we just throw here?
