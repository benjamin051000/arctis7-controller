...
// Credit Nevermore on stack overflow:
// https://stackoverflow.com/a/32763779/11074982
...
struct posix_duration_cast<std::chrono::duration<Rep, Period>, struct timeval> {
    static struct timeval cast(std::chrono::duration<Rep, Period> const& d) {
        ...
        tv.tv_sec = sec.count();
        tv.tv_usec =
            std::chrono::duration_cast<std::chrono::microseconds>(d - sec)
                .count();
...
template <typename Rep, typename Period>
struct posix_duration_cast<struct timeval,
...
auto duration_cast(std::chrono::duration<Rep, Period> const& d)
    -> std::enable_if_t<std::is_same<T, struct timeval>::value,
                        struct timeval> {
